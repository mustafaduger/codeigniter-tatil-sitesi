...
class Categoryexplore_controller extends CI_Controller {
	public function __construct()
        {
        		parent::__construct();
                 ...
                 redirect('auth/login');
           		...
           		$this->load->model('categoryexplore_model');
	...
	public function index()
	{
		...
		$data['result']=$this->categoryexplore_model->getAll_category();
		$this->load->view('admin/explore/category/category',$data);
	...
	public function add_category_page_controller()
	{
		...
		$this->load->view('admin/explore/category/add/add_category');
		
	}

	public function add_category_controller()
	{	
    $this->form_validation->set_rules('kategori_adi', 'Ad', 'required');
		...
		else
		{
			$data = array(
           	...
           	$result=$this->categoryexplore_model->add_category($data);
               ...
               <h4><i class="icon fa fa-check"></i> Başarılı!</h4>
                Kayıt eklendi. 
      			...
      			redirect('categoryexplore_controller');
           	}else{
	...
	{
		$result=$this->categoryexplore_model->delete_category($id);
			   if ($result) {
           		 $this->session->set_flashdata('login_error','<div class="alert alert-success alert-dismissible">
         			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
               <h4><i class="icon fa fa-check"></i> Başarılı!</h4>
  ...
  {
    $result=$this->categoryexplore_model->edit_category($id);
    $data['result']=$result;
    $this->load->view('admin/explore/category/edit/edit_category',$data);
  ...
  public function update_category_controller()
  {
    ...
    //print_r($data);
    $result=$this->categoryexplore_model->update_category($id,$data);
    //print_r($result);
          if ($result) {
               $this->session->set_flashdata('login_error','<div class="alert alert-success alert-dismissible">
              <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
  ...
  public function isActive_set()
    {
      ...
      $this->db->where('kategori_id',$id)->update('tbl_kategorikesfet',array('kategori_isActive'=>$isActive));
    }
